...
public class DA_MP_TC017_User_Can_Remove_Any_Main_Parent_Page_Except_Overview_Page_Successfully_And_The_Order_Of_Pages_Stays_Persistent_As_Long_As_There_Is_Not_Children_Page_Under_It extends BaseTest {
	...
	public void DA_MP_TC017(Hashtable<String, String> data) throws Exception {
		...
		Assert.assertTrue(homePage.isPageDisplayed(data.get("page_name")), String.format("%s isn't displayed",data.get("page_name_1")));
		...
		Assert.assertTrue(homePage.isChildPageDisplayed(data.get("page_name_child")), String.format("%s isn't displayed",data.get("page_name_1")));
		...
		Assert.assertEquals(homePage.getAlterMessage(), data.get("confirm_message"), "The confirm message doesn't match");
		
		Log.info("Step #9: Click OK button");
		DriverUtils.acceptAltert();
		
		Log.info("Step #10: Check warning message \"Can not delete page 'Test' since it has children page(s)\" appears");
		Assert.assertEquals(homePage.getAlterMessage(), data.get("warning_message"), "The warning message doesn't match");
		...
		Assert.assertFalse(homePage.isChildPageDisplayed(data.get("page_name_child")),String.format("%s page should not dispaly", data.get("page_name_child")));
		...
		Assert.assertFalse(homePage.isPageDisplayed(data.get("page_name")),String.format("%s page should not dispaly", data.get("page_name")));
		
		Log.info("Step #22: Click on \"Overview\" page");
		homePage.selectMenuPage("Overview");
		
		Log.info("Step #23: Check \"Delete\" link disappears");
		Assert.assertFalse(homePage.isItemUnderSettingDisplayed(SettingsButton.DELETE_PAGE.getName()), String.format("%s is not display", SettingsButton.DELETE_PAGE.getName()));
